...
def refresh_single_topic(topic_id: int):
    ...
    pipeline = chain(
        group(
            ingest_govuk_for_topic.si(topic_id),
            ingest_parliament_for_topic.si(topic_id),
        ),
...
def daily_refresh_all_topics():
    """Daily refresh: ingest all topics in parallel, then match and rebuild graph."""
    with get_sync_session() as db:
        topic_ids = [t.id for t in db.query(Topic).all()]

    if not topic_ids:
        logger.info("No topics configured, skipping daily refresh")
        return {"status": "no_topics"}

    # Phase 1: Ingest all topics in parallel (GOV.UK + Parliament per topic)
    ingest_tasks = group(
        group(
            ingest_govuk_for_topic.si(tid),
            ingest_parliament_for_topic.si(tid),
        )
    ...
    logger.info("Daily refresh pipeline started for %d topics", len(topic_ids))
